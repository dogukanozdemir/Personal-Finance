...
const Import = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await importAPI.importTransactions(files);
      setResult(response.data);
      
      // Update status based on results
      if (response.data.fileResults) {
        response.data.fileResults.forEach((fileResult: any) => {
          const status = fileResult.errors && fileResult.errors.length > 0 ? 'error' : 'success';
          setProcessingStatus(prev => ({...prev, [fileResult.fileName]: status}));
        });
      }
    } catch (err: any) {
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div>
                  <p className="text-text-muted text-sm">Rows Parsed</p>
                  <p className="text-2xl font-bold">{result.totalRowsParsed}</p>
                </div>
                <div>
                  <p className="text-text-muted text-sm">New Transactions</p>
                  <p className="text-2xl font-bold text-success">{result.totalInserted}</p>
                </div>
                <div>
                  <p className="text-text-muted text-sm">Duplicates Skipped</p>
                  <p className="text-2xl font-bold text-accent">{result.totalSkippedDuplicates}</p>
                </div>
              </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    {result.fileResults.map((fileResult: any, index: number) => (
                      <div key={index} className="p-3 bg-background rounded-lg border border-gray-700">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <p className="text-sm font-medium">{fileResult.fileName}</p>
                            <div className="flex space-x-4 mt-2 text-xs text-text-muted">
                              <span>Parsed: {fileResult.rowsParsed}</span>
                              <span className="text-success">Inserted: {fileResult.inserted}</span>
                              <span className="text-accent">Skipped: {fileResult.skippedDuplicates}</span>
                            </div>
                            {fileResult.errors && fileResult.errors.length > 0 && (
                              <div className="mt-2">
                                <p className="text-xs text-danger font-semibold mb-1">Errors:</p>
                                <ul className="text-xs text-danger list-disc list-inside space-y-0.5">
                                  {fileResult.errors.map((error: string, errIndex: number) => (
